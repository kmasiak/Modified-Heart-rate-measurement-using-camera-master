...
class Process(object):
    ...
    def run(self):
        ...
        if self.process_this_frame:
            try:
                self.frame, self.face_frame, self.ROI1, self.ROI2, self.status, self.mask = self.fd.face_detect_artery(
                    self.frame_in)
            except TypeError:
                print("end video")
        ...
        if self.framecount % 10 == 0:
            #print('@ ', self.framecount, ' ', self.mean10count)
            self.mean10count.clear()
        else:
            self.mean10count.append(self.bpms)
        if self.status:
            self.facecount += 1
