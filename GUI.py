...
class GUI(QMainWindow, QThread):
    ...
    def main_loop(self):
        ...
        try:
            ...
            mean10 = 0
            for i in range(int(len(self.kylebpm) / 10)):
                for j in range(10):
                    mean10 += self.kylebpm[i * 10 + j]
                print(mean10/10)
                mean10 = 0
            ...
            print('Mean', mean(self.kylebpm))
            print('Standard Deviation', np.std(self.kylebpm))
            x = np.array(range(0, len(self.kylebpm)))
            y = self.kylebpm
            print('x', x)
            print('y', y)
            plt.plot(x, y)
        ...
        self.kylebpm.append(float("{:.2f}".format(self.bpm)))
    ...
    def run(self, input):
        ...
        if self.status == False:
            ...
            #self.kylebpm = [i for i in self.kylebpm if i != 0]
